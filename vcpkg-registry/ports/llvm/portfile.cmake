...
vcpkg_from_github(
    ...
    SHA512 cdbddfa241f9681610a551ad77db6cb9d8ae29031fbfc9d399aaf41a9e672da5b9ccc57126221a22833f1badf0fe67dd5564560c482a6e9503711db85491e195
    HEAD_REF master
    PATCHES
            ...
            foreach(_var ${data_list})
                if("${_var}" MATCHES "^(ID)=(.*)$")
                    if("${CMAKE_MATCH_2}" MATCHES "alpine")
                        set(HOST_IS_MUSL TRUE)
                    else()
                        set(HOST_IS_MUSL FALSE)
                    endif()
                endif()
            endforeach()
            if(HOST_IS_MUSL)
                message(STATUS "Musl detected, using custom options for LLVM")
                list(APPEND CROSS_OPTIONS -DLLVM_HOST_TRIPLE=x86_64-alpine-linux-musl)
                list(APPEND CROSS_OPTIONS -DLLVM_DEFAULT_TARGET_TRIPLE=x86_64-alpine-linux-musl)
                list(APPEND CROSS_OPTIONS -DLIBUNWIND_TARGET_TRIPLE=x86_64-alpine-linux-musl)
                list(APPEND CROSS_OPTIONS -DCOMPILER_RT_DEFAULT_TARGET_TRIPLE=x86_64-alpine-linux-musl)
